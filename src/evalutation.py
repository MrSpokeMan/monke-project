...
class Evaluation:
    ...
    def _fill_dataset(self, outputs):
        for output in tqdm(outputs, desc="Evaluating questions", unit="eval_q"):
            evaluations = {
                "groundedness": self.call_llm(QA_CRITIQUE_GROUNDEDNESS.format(question=output["question"], context=output["context"])),
                "relevance": self.call_llm(QA_CRITIQUE_RELEVANCE.format(question=output["question"])),
                "standalone": self.call_llm(QA_CRITIQUE_STANDALONE.format(question=output["question"]))
            }
            try:
...
def _remove_low_scores(outputs):
    ...
    for output in outputs:
        if output["groundedness_score"] and output["relevance_score"] and output["standalone_score"]:
