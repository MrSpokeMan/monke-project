    ...
    for div in subdivisions:
        ...
        for i, p in enumerate(paragraphs):
            if i % 2 == 1:  # Assume odd paragraphs are the main content
                points.append({"name": name, "text": p.get_text(strip=True)})
    ...
    for p in all_paragraphs:
        ...
        if "ti-art" in classes:
            if collecting and current_article_text.strip():
                points.append({"name": name, "text": current_article_text.strip()})
            collecting = True
            current_article_text = ""
        ...
        while current:
            if current.name == "table":
                ...
                if combined_text.strip():
                    points.append({"name": name, "text": combined_text.strip()})
